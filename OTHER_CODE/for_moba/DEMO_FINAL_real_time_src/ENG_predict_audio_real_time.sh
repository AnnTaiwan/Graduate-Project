...
while true; do
    ...
    if ls $AUDIO_FOLDER/*.wav 1> /dev/null 2>&1; then
        for audio_file in $AUDIO_FOLDER/*.wav; do
            ...
            sh denoise_add_volume_ENG.sh $FIX_AUDIO_FOLDER/audio_fix.wav $NOISE_REDUCE_AUDIO_FOLDER/audio_denoise.wav $PROFILE/audio_profile.prof
            ...
            ./demo_final_segment_audio_ver2 CNN_model13_ENG_netname.xmodel $IMG_FOLDER/ detail_result_ENG.txt
            echo "Finished predicting $audio_file."
            echo "## Statistics"
            cat Result.txt
            echo
            cat detail_result_ENG.txt
