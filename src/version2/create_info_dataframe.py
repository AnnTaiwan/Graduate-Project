...
if __name__ == "__main__":
    train_df = pd.read_csv(f'{BASE_PATH}/ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.eval.trl.txt',
                        sep=" ", header=None)

    train_df.columns =['speaker_id','filename','system_id','null','class_name']
    train_df.drop(columns=['null'],inplace=True)

    train_df['filepath'] = f'{BASE_PATH}/ASVspoof2019_LA_eval/flac/'+train_df.filename+'.flac'
    train_df['target'] = (train_df.class_name=='spoof').astype('int32')
    # output the info
    print("Some information about LA eval:")
    print('\tLen eval', len(train_df))
    ...
    # get the csv file
    train_df.to_csv('eval_info.csv', index=False)